...
the member name. */
SELECT DISTINCT CONCAT_WS(" ", m.firstname, m.surname ) AS fullname, sub.name
FROM Members AS m
LEFT JOIN (Select f.name, b.memid
           FROM Facilities AS f
           LEFT JOIN Bookings AS b
           ON f.facid = b.facid
           WHERE f.facid IN (0,1)) AS sub
...
Order by descending cost, and do not use any subqueries. */
    SELECT * FROM (
    SELECT SUM(cost) AS total, fullname FROM (
SELECT  ( m.firstname ||" " || m.surname ) AS fullname,
CASE WHEN m.memid > 0 THEN f.membercost * b.slots
ELSE f.guestcost * b.slots END AS cost, f.name, b.starttime
FROM Bookings AS b
...
I believe I had to use a subquery last time
...
SELECT m1.firstname, m1.surname, m1.recommendedby, m2.firstname, m2.surname
FROM Members AS m1
...
SELECT COUNT(month), facid, facility
FROM (
SELECT EXTRACT(MONTH FROM starttime) AS month, f.name AS facility, f.facid
FROM Bookings AS b
LEFT JOIN Facilities AS f
USING (facid)) AS sub
GROUP BY facid
